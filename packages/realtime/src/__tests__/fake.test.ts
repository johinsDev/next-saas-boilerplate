...
describe("FakeRealtime", () => {
  it("records every publish", async () => {
    ...
    await fake.publish("user:u_1", { event: "subscription.started", data: { n: 3 } });
    await fake.publish("user:u_1", { event: "entitlement.ready", data: {} });
  ...
  it("assertPublished matches by room + predicate", async () => {
    const fake = new FakeRealtime();
    await fake.publish("user:u_1", { event: "subscription.started", data: { n: 5 } });
    fake.assertPublished("user:u_1", (e) => e.data.n === 5);
    expect(() =>
      fake.assertPublished("user:u_1", (e) => e.data.n === 99),
    ).toThrow();
    expect(() => fake.assertPublished("user:u_OTHER")).toThrow();
  });

  it("assertPublishedCount", async () => {
    const fake = new FakeRealtime();
    await fake.publish("user:u_1", { event: "a", data: {} });
    await fake.publish("user:u_1", { event: "b", data: {} });

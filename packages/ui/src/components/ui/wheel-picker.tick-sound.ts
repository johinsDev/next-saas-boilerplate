// Synthesizes a short, dry mechanical detent for gesture feedback (the
...
// `createTickPlayer()` caller. A DateWheelPicker alone mounts three wheels;
// giving each its own AudioContext would waste resources and risk hitting a
// browser's cap on concurrent contexts. The context only closes once every
// consumer that created a player has called `dispose()`.
...
function buildClickBuffer(context: AudioContext): AudioBuffer {
  ...
  for (let i = 0; i < length; i++) {
    ...
    const sample = (resonance + transient) * attack * fadeOut;
    data[i] = sample;

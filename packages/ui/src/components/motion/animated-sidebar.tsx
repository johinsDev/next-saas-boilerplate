    ...
    >
      ...
      <motion.div
        ...
        onKeyDown={(event) => {
          ...
          // `?.` rather than `!`: our tsconfig sets `noUncheckedIndexedAccess`,
          // which beUI's does not, so an index read is `T | undefined` here.
          // The early return above already proves the list is non-empty — this
          // only tells the compiler so. Reapply after `shadcn add --overwrite`.
...
export interface AnimatedSidebarMenuSubButtonProps {
  ...
  onSelect?: () => void;
  className?: string;
  /** See `AnimatedSidebarMenuButtonProps.linkAs`. */
  linkAs?: ElementType;
}
...
export function AnimatedSidebarMenuSubButton({
  ...
  const Anchor = (linkAs ?? motion.a) as ElementType;
...
export interface AnimatedSidebarMenuButtonProps {
  ...
  className?: string;
  /**
   * Router-aware link component to render instead of a plain anchor.
   *
   * beUI is framework-agnostic, so `href` renders `<a>` — which is a full page
   * load: no client-side navigation, no prefetch, and every provider above it
   * remounts. Pass `next/link` here to get all three back.
   *
   * Hoist the value to module scope. A component created during render is a new
   * type every render, so React unmounts and remounts the subtree.
   */
  linkAs?: ElementType;
}

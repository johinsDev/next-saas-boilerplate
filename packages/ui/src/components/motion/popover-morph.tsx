...
export interface MorphPopoverContentProps {
  ...
  className?: string;
  /**
   * Classes for the wrapper the panel scales inside. The shadow belongs here
   * as a `drop-shadow` filter: the panel itself is clipped while it opens, and
   * a `box-shadow` on it would be clipped away with it.
   */
...
export function MorphPopoverContent({
  ...
  radius = 16,
  className,
  wrapperClassName = "[filter:drop-shadow(0_10px_18px_rgba(0,0,0,0.14))]",
  ...
  const { left, top } = layout
    ? placePopover(layout, side, align, sideOffset, {
    ...
    <AnimatePresence>
      {ctx.open ? (
        <motion.div
          ...
          className={cn("fixed z-[9999]", wrapperClassName)}
        >
          <motion.div
            ref={ctx.contentRef}
            id={ctx.contentId}

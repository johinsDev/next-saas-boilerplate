...
export function BottomSheet({
  ...
  const onDragEnd = (_: unknown, info: PanInfo) => {
    ...
    if (velocity > 600 || offset > dismissThreshold) {
      ...
      // `smaller.at(-1)` rather than `smaller[smaller.length - 1]`: our tsconfig
      // sets `noUncheckedIndexedAccess`, and beUI's does not. The `smaller.length`
      // guard above already proves there is one; `?? 0` only says so to the
      // compiler, and 0 is the closed-most snap either way.
      if (smaller.length && velocity < 800 && offset < dismissThreshold * 1.6) {
        setSnap(smaller.at(-1) ?? 0);
      } else {
        onOpenChange(false);
      }
      return;
  ...
  /*
   * `snap` is an index into `snapPoints`, and every path that sets it clamps
   * to the array — but `noUncheckedIndexedAccess` cannot see that, and a
   * `snapPoints` prop shorter than a stale `snap` would genuinely be
   * undefined. Falling back to the first point is the same thing the component
   * opens with.
   */
  const snapValue = snapPoints[snap] ?? snapPoints[0] ?? "auto";
  const heightStyle =
    snapValue === "auto"
      ? { maxHeight: "92vh" }
